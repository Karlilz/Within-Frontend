...
import JournalModal from '../components/JournalModal.js'; 
    ...
    <div>
      <div style={containerStyle}>
        ...
        <h2 style={{color:"white", fontFamily: 'Staatliches', fontSize:"40px", textDecoration:"underline"}}>
          ...
          <button onClick={() => setShowModal(true)} style={{ marginLeft: '10px' }}>Create Journal Entry</button>
        </h2>
        ...
        <JournalModal
          ...
          handleSaveEntry={handleAddEntry}
        />
            ...
            {journalEntries.map((entry) => (
              <li key={entry._id}>
                {editingEntryId === entry._id ? (
                  ...
                  <>
                    <strong style={{color:'white'}}> Title: {entry.title}</strong>
                    <p style={{color:'white'}}>{entry.content}</p>
                    <p style={{ color: 'white' }}>Date: {new Date(entry.date).toLocaleDateString()}</p>
